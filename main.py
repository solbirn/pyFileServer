...
# Content type sources taken from http://en.wrikipedia.org/wiki/MIME_type
content_type_table = {'js': 'application/x-javascript', 'html': 'text/html; charset=utf-8',
                      ...
                      'mpeg':'video/mpeg', 'mp4':'video/mp4', 'qt':'video/quicktime', 'mov':'video/quicktime', 'rar':'application/x-rar-compressed',
...
class FileResponse(object):
    readsize = 1024
    def __init__(self, f, filename, environ):
        self.size = os.path.getsize(filename)
        self.f = f
        self.range = None
        if environ.get('HTTP_RANGE'):
            temp = environ['HTTP_RANGE'].split('=')[-1].split('-')
            if temp[1] == '': temp[1]=str(self.size)
            print temp
            self.range = {'start':int(temp[0]),'end':int(temp[1]),'total':int(temp[1])-int(temp[0])}
            self.range.update({'extrachunk':self.range['total']%self.readsize})
            self.size = self.range['total']
    def __iter__(self):
        if self.range:
            self.f.seek(self.range['start'])
            output = '\n'
            read = 0
            while (len(output) is not 0): #and (read < self.range['end']-self.range['extrachunk']):
                output = self.f.read(self.readsize)
                yield output
                logger.error(len(output))
                #read+=self.readsize
            yield self.f.read(self.range['extrachunk'])
        else:
            output = '\n'
...
class WSGIFileServerApplication(object):
    ...
    def handler(self, environ, start_response):
        ...
        full_path = os.path.join(self.path, serve_file)
        print full_path
        print environ
                
        def do_get():
            if serve_file.endswith('/') or os.path.isdir(full_path):
                if os.path.isdir(full_path):
                    ...
                    return [ '<html>' + 
                              '<br>'.join( [('<a href="%s:////%s/%s">%s</a>' % (url[0],url[1]+url[2], filename, filename)).replace('//','/')
            ...
            response = FileResponse(f, os.path.join(self.path, serve_file), environ)
            start_response('200 OK', [('Cache-Control','no-cache'), ('Pragma','no-cache'), 
...
if __name__ == "__main__":
    ...
    args = {'port':5678, 'host':'127.0.0.1', 'mountpoint':None}
    args.update({'path':'C:\\'})#,'mountpoint':'//c//'})
